...
class BaseTrainer:
    ...
    def train(self):
        ...
        elif torch.mlu.is_available():  # i.e. device=None or device='' or device=number
    ...
    def _setup_ddp(self, world_size):
        ...
        self.device = torch.device('mlu', RANK)
        # LOGGER.info(f'DDP info: RANK {RANK}, WORLD_SIZE {world_size}, DEVICE {self.device}')
        # os.environ['NCCL_BLOCKING_WAIT'] = '1'  # set to enforce timeout
        dist.init_process_group(
            'cncl' if dist.is_cncl_available() else 'gloo',
    ...
    def _setup_train(self, world_size):
        ...
        self.scaler = torch.mlu.amp.GradScaler(enabled=self.amp)
        if world_size > 1:
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                # Forward
                with torch.mlu.amp.autocast(self.amp):
            ...
            torch.mlu.empty_cache()  # clear GPU memory at end of epoch, may help reduce CUDA out of memory errors
