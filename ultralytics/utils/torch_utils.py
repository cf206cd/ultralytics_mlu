...
def select_device(device='', batch=0, newline=False, verbose=True):
    ...
    if cpu or mps:
        ...
        #os.environ['CUDA_VISIBLE_DEVICES'] = -1  # set environment variable - must be before assert is_available()
        if not (torch.mlu.is_available() and torch.mlu.device_count() >= len(device.replace(',', ''))):
            LOGGER.info(s)
            install = 'See https://pytorch.org/get-started/locally/ for up-to-date torch install instructions if no ' \
                      'CUDA devices are seen by torch.\n' if torch.mlu.device_count() == 0 else ''
            raise ValueError(f"Invalid CUDA 'device={device}' requested."
                             f" Use 'device=cpu' or pass valid CUDA device(s) if available,"
                             f" i.e. 'device=0' or 'device=0,1,2,3' for Multi-GPU.\n"
                             f'\ntorch.mlu.is_available(): {torch.mlu.is_available()}'
                             f'\ntorch.mlu.device_count(): {torch.mlu.device_count()}'
